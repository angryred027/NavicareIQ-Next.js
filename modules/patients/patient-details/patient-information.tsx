...
import { Card, Divider, PatientCard } from '@/components/common';
...
const PatienInformationContent = () => {
  return (
...
const PatientInformation: FC<PatientInformationProps> = ({ hasAccordion = false }) => {
  return (
    <div>
      {!hasAccordion && (
        <>
      ...
      {hasAccordion && (
        <Card className="p-[0px]">
          <Accordion title={<PatientCard name="Angelina Perreira" gender="Female" age={32} />} variant="v2">
            <PatienInformationContent />
          </Accordion>
        </Card>
      )}
      {!hasAccordion && <PatienInformationContent />}
    </div>
  );
};
