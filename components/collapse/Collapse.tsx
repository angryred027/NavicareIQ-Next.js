...
const Collapse = ({ title, subTitle, children, defaultOpen = false }: CollapseProps) => {
  const [isOpen, setIsOpen] = useState(false);

  return (
    <div className={'w-full border-b border-system-light-200 overflow-hidden '}>
      <button className="w-full p-4 flex justify-between" onClick={() => setIsOpen(!isOpen)}>
        ...
        <div className="flex items-center justify-center">
          {!isOpen ? <Icon name="arrowDown" /> : <Icon name="arrowDown" className="rotate-180" />}
        </div>
      </button>
